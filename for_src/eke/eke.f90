...
!=======================================================================
 ...
 do k=1,nz
  do j=js_pe,je_pe
   do i=is_pe-1,ie_pe
      flux_east(i,j,k)=0.5*max(500d0,K_gm(i,j,k)+K_gm(i+1,j,k))* &
             (eke(i+1,j,k,tau)-eke(i,j,k,tau))/(cost(j)*dxu(i))*maskUtr(i,j,k)
   enddo
  enddo
  do j=js_pe-1,je_pe
   do i=is_pe,ie_pe
      flux_north(i,j,k)=0.5*max(500d0,K_gm(i,j,k)+K_gm(i,j+1,k))* &
            (eke(i,j+1,k,tau)-eke(i,j,k,tau))/dyu(j)*maskVtr(i,j,k)*cosu(j)
   enddo
  enddo
 enddo
 ...
 if (enable_eke_dst3_advection) then
  call adv_flux_dst3_wgrid(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,nz,flux_east,flux_north,flux_top,eke(:,:,:,tau))
 endif
 if (enable_eke_superbee_advection .or. enable_eke_dst3_advection) then
  do j=js_pe,je_pe
   do i=is_pe,ie_pe
  ...
  if (enable_eke_AB_time_stepping) then
    !---------------------------------------------------------------------------------
    ! Adam Bashforth time stepping
    !---------------------------------------------------------------------------------
     eke(:,:,:,taup1)=eke(:,:,:,taup1)+dt_tracer*( (1.5+AB_eps)*deke(:,:,:,tau) &
          - ( 0.5+AB_eps)*deke(:,:,:,taum1))
  else
     eke(:,:,:,taup1)=eke(:,:,:,taup1)+dt_tracer*deke(:,:,:,tau) 
  endif
  !---------------------------------------------------------------------------------
  ! open boundaries
  !---------------------------------------------------------------------------------
  call set_obc_tracer_wgrid(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,nz,eke,.true.) 
 endif

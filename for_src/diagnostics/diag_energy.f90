...
!=======================================================================
 ...
 real*8 :: mdiss_ide
 real*8 :: IW_lee_p,IW_lee_m,IW_mean_to_lee,IW_lee_flux,dIW_lee_p,dIW_lee_m,IW_lee_to_iw
 ...
 !---------------------------------------------------------------------------------
 ! vertical friction by idemix
 !---------------------------------------------------------------------------------
   mdiss_ide=0; 
   if (enable_idemix .and. (enable_idemix3.or.enable_leewaves)) then
    do k=1,nz 
     do j=js_pe,je_pe
       do i=is_pe,ie_pe
        fxa = area_t(i,j)*dzt(k)*maskT(i,j,k)
        mdiss_ide = mdiss_ide+ K_diss_idemix(i,j,k)*fxa
       enddo
     enddo
    enddo
    call global_sum(mdiss_ide)
   endif
 ...
 !---------------------------------------------------------------------------------
   ...
   if (enable_idemix .and. enable_idemix3) then
    do k=1,nz 
      do j=js_pe,je_pe
       do i=is_pe,ie_pe
        fxa = area_u(i,j)*dzt(k)*maskU(i,j,k)
        IWm = IWm + fxa*(F_s(i,j,k,tau)+G_s(i,j,k,tau))
        dIWm = dIWm + fxa*(F_s(i,j,k,taup1)-F_s(i,j,k,tau) + G_s(i,j,k,taup1)-G_s(i,j,k,tau) )/dt_tracer
        fxa = area_v(i,j)*dzt(k)*maskV(i,j,k)
        IWm = IWm + fxa*(O_s(i,j,k,tau)+P_s(i,j,k,tau))
        dIWm = dIWm + fxa*(O_s(i,j,k,taup1)-O_s(i,j,k,tau) + P_s(i,j,k,taup1)-P_s(i,j,k,tau) )/dt_tracer
        fxa = area_t(i,j)*dzw(k)*maskW(i,j,k)
        if (k==nz) fxa=fxa*0.5
        IWdiss = IWdiss + fxa*iw_diss(i,j,k)
       enddo
     ...
     do j=js_pe,je_pe
      do i=is_pe,ie_pe
        ...
        if (k>0) IWforc=IWforc + 0.5*area_u(i,j)*(forc_iw_surface_u(i,j)*maskU(i,j,nz) &
                                                  +forc_iw_bottom_u(i,j)*maskU(i,j,k))
        k=max(kbot(i,j),kbot(i,j+1))
        if (k>0) IWforc=IWforc + 0.5*area_v(i,j)*(forc_iw_surface_v(i,j)*maskV(i,j,nz) &
                                                  +forc_iw_bottom_v(i,j)*maskV(i,j,k))
      enddo
     enddo
     call global_sum(IWm); call global_sum(dIWm); call global_sum(IWdiss); call global_sum(IWforc);
   endif
  !---------------------------------------------------------------------------------
  ! Lee wave compartment
  !---------------------------------------------------------------------------------
   IW_lee_p = 0;IW_lee_m=0;dIW_lee_p=0;dIW_lee_m=0;IW_lee_flux=0;
   IW_mean_to_lee=0;IW_lee_to_iw=0
   if (enable_idemix .and. enable_leewaves) then
    ...
    do k=1,nz-1 
      do j=js_pe,je_pe
       do i=is_pe,ie_pe
        fxa = area_t(i,j)*dzw(k)*maskW(i,j,k)
        !if (k==nz) fxa=fxa*0.5
        IW_lee_p = IW_lee_p + fxa*E_lee_p(i,j,k,tau)
        IW_lee_m = IW_lee_m + fxa*E_lee_m(i,j,k,tau)
        dIW_lee_p = dIW_lee_p + fxa*(E_lee_p(i,j,k,taup1)-E_lee_p(i,j,k,tau))/dt_tracer
        dIW_lee_m = dIW_lee_m + fxa*(E_lee_m(i,j,k,taup1)-E_lee_m(i,j,k,tau))/dt_tracer
        IW_mean_to_lee = IW_mean_to_lee + fxa*mean_flow_to_E_lee(i,j,k)
        IW_lee_to_iw = IW_lee_to_iw + fxa*iw_diss_lee(i,j,k)
       enddo
      enddo
     enddo
     do j=js_pe,je_pe
      do i=is_pe,ie_pe
        k=max(1,kbot(i,j))
        IW_lee_flux=IW_lee_flux + area_t(i,j)*flux_lee_tot(i,j)*maskW(i,j,k)
      enddo
     enddo

     call global_sum(IW_lee_p); call global_sum(IW_lee_m);call global_sum(dIW_lee_p);
     call global_sum(dIW_lee_m);call global_sum(IW_lee_flux);call global_sum(IW_mean_to_lee);
     call global_sum(IW_lee_to_iw);
   endif
 !---------------------------------------------------------------------------------
 ...
 !---------------------------------------------------------------------------------
   ...
   mean_diss(1)   = mean_diss(1) + mdiss_h+mdiss_v+mdiss_gm+mdiss_bot+mdiss_ide
   ...
   mean_exchg(5) = mean_exchg(5) - mdiss_vmix           ! TKE-> Hd
   if (enable_take_p_diss_adv_from_tke)  mean_exchg(5) = mean_exchg(5) -mdiss_adv
   ...
   if (enable_idemix .and. (enable_idemix3 .or. enable_leewaves)) then
      mean_exchg(6) = mean_exchg(6) + mdiss_ide         ! KE -> IW
      
    
      !mean_misc(30) = mean_misc(30) + mdissF_ide         
      !mean_misc(31) = mean_misc(31) + mdissG_ide         
      !mean_misc(32) = mean_misc(32) + mdissO_ide         
      !mean_misc(33) = mean_misc(33) + mdissP_ide         
   endif
   ...
   if (enable_idemix .and. enable_leewaves) then
        mean_e(30) = mean_e(30) + IW_lee_p
        mean_e(31) = mean_e(31) + IW_lee_m
        mean_dedt(30) = mean_dedt(30) + dIW_lee_p
        mean_dedt(31) = mean_dedt(31) + dIW_lee_m
        mean_misc(30) = mean_misc(30) + IW_lee_flux
        mean_misc(31) = mean_misc(31) + IW_mean_to_lee
        mean_misc(32) = mean_misc(32) + IW_lee_to_iw
   endif
...
!=======================================================================
 ...
 if (my_pe==0) then
    ...
    if (enable_idemix .and. enable_leewaves) &
     print'(a,f12.3,a)',' E_lee        ',(mean_e(30)+mean_e(31))*rho_0/1d15,' PetaJ'
    ...
    if (enable_idemix) then
      ...
      print'(a,e12.6,a)',' EKE ->  IW     ',mean_exchg(8)*fxa,'  TW'
      fxb = mean_dedt(4)-mean_forc(4)+mean_diss(4)-mean_exchg(8)
      if (enable_idemix3) then
       print'(a,e12.6,a)',' KE  ->  IW     ',mean_exchg(6)*fxa,'  TW'
       fxb = fxb - mean_exchg(6)
      endif    
      if (enable_idemix_niw) fxb = fxb - mean_diss(6)
      if (enable_leewaves) then
        print'(a,e12.6,a)',' E_lee -> IW    ',mean_misc(32)*fxa,'  TW'
        fxb = fxb - mean_misc(32)
      endif   
    ...
    if (enable_idemix .and. enable_leewaves) then
     print'(a)',' Lee wave energy budget:'
     print'(a,e12.6,a)',' dE_lee/dt    ',(mean_dedt(30)+mean_dedt(31))*fxa,'  TW'
     print'(a,e12.6,a)',' Lee wave flux  ',mean_misc(30)*fxa,'  TW'
     print'(a,e12.6,a)',' KE ->Lee waves ',mean_misc(31)*fxa,'  TW'
     print'(a,e12.6,a)',' E_lee -> E_iw  ',mean_misc(32)*fxa,'  TW'
     fxb = mean_dedt(30)+mean_dedt(31) -mean_misc(30) - mean_misc(31) + mean_misc(32)
     print'(a,e12.6,a)',' error in dE/dT ',fxb*fxa,'  TW'
     fxb = mean_exchg(6)-mean_misc(30)-mean_misc(31)
     print'(a,e12.6,a)',' error in exchg ', fxb*fxa,'  TW'   
     print*,' '
    endif
    ...
    if (.not. enable_take_p_diss_adv_from_tke) fxb = fxb - mean_misc(5)
    print'(a,e12.6,a)',' error          ',fxb*fxa,'  TW'
    print*,' '


    print'(a,a)',' writing energy diagnostics to file energy.cdf'
    iret=nf_open('energy.cdf',NF_WRITE,ncid)
...
!=======================================================================
 ...
 if (my_pe==0) then
    ...
    print'(a,a)',' preparing file energy.cdf'
      iret = nf_create ('energy.cdf', nf_clobber, ncid)

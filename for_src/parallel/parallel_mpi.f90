...
subroutine zonal_sum_vec2(a,len)
!--------------------------------------------------------------
!    sum vector along zonal row of PEs
!    result is communictated to all PEs of row
!--------------------------------------------------------------
      use main_module   
      implicit none
      integer, intent(in) :: len
      ...
      call zonal_sum_vec(a,len)
      if (n_pes_i>1) then
        do n=2,n_pes_i
          if (my_blk_i == 1) then
            call mpi_send(a,len,mpi_real8,my_pe+(n-1),tag,my_comm,ierr)
          else
            if (my_blk_i==n) call mpi_recv(a,len,mpi_real8,my_pe-(my_blk_i-1),tag,my_comm,Status,ierr)
          endif
        enddo
      endif 
end subroutine zonal_sum_vec2

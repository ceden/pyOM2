...
!----------------------------------------------------------------------
        ...
        fxa = dot_sfp(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,p,Ap)
        if (fxa == 0d0) goto 101
        alpha=rsold/fxa ! dot_sfp(is_pe-onx,ie_pe+onx,js_pe-onx,je_pe+onx,p,Ap)
...
!-----------------------------------------------------------------------
        ...
        elseif (n .gt. 2) then
          ...
          if (abs(rsnew) .gt. 100.0*rs_min) then
           if (my_pe==0) print*,'WARNING: solver for surface pressure is diverging after ',n,' iterations'
           goto 99
          endif
        endif
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
        if (estimated_error/=estimated_error ) then
          if (my_pe==0) print*,'Warning: estimated error for surface pressure is NaN at iteration step ',n
          goto 99
        endif
    enddo
    if (my_pe==0) print*,' WARNING: max iterations for surface pressure exceeded at itt=',itt
    goto 99
...
99  iterations = n  ! error end
   ...
   ! check for NaN
    if (estimated_error/=estimated_error) then
        if (my_pe==0) print'(/a/)',' surface pressure error is NaN, stopping integration '
        call panic_snap

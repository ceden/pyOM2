...
!=======================================================================
 ...
 integer, parameter :: maxipp = 20000, mnisle = 1000
 ...
 if (enable_cyclic_y) then
    if (my_pe==0) print*,' ERROR: cyclic meridional boundary conditions not yet implemented'
 ...
 do isle=1,nisle
   ...
   do j=1-onx,ny+onx
      do i=1-onx,nx+onx
         if (allmap(i,j) == isle ) cont = .true.
      enddo
   enddo
   if (.not. cont) then
      if (my_pe==0) then
         print*,' WARNING: cannot find island ',isle,' in map'
         print*,' iofs= ',iofs(isle)
         print*,' nippts= ',nippts(isle)
         print*,' iperm= ',iperm(iofs(isle):iofs(isle)+nippts(isle) )
         print*,' jperm= ',jperm(iofs(isle):iofs(isle)+nippts(isle) )
      endif
      call fortran_barrier
      call halt_stop(' in streamfct_ini: cannot find island in map')
   endif
 enddo
 !-----------------------------------------------------------------------
 ! allocate variables
 !-----------------------------------------------------------------------
 max_boundary= 20*maxval(nippts(1:nisle))
 ...
 do isle=1,nisle
   ...
   if ( .not.cont ) then
    ...
    if ( .not.cont ) then
       print'(a,i3,a,i3)','PE',my_pe,': found no starting point for line integral for island ',isle
       call fortran_barrier
       call halt_stop('in streamfunction_init: found no starting point')
    endif
   ...
   do while (cont) 
     ...
     if (map(ijp(1),ijp(2))==-1 .and. map(ijp_right(1),ijp_right(2)) == 1 ) then
      ...
      print'(a,i3,a,i3)','PE',my_pe,': unknown situation or lost track for island ',isle
      print'(a,i3,a,i3,a,i3)','PE',my_pe,': current position i=',ij(1),' j=',ij(2)
      do n=1,n
       print*,' pos=',boundary(isle,n,:),' dir=',line_dir(isle,n,:)
      enddo
      print*,' map ahead is ',map(ijp(1),ijp(2)) , map(ijp_right(1),ijp_right(2)) 
      call fortran_barrier
      call halt_stop(' in streamfunction_init: lost track ')
     endif
     ...
     if (cont) then
       ...
       if (n>max_boundary) then
         print'(a,i3,a,i8)','PE',my_pe,': increase value of max_boundary, currently ',n
         call fortran_barrier
         call halt_stop(' in streamfunction_init: increase max_boundary ')
       endif
       boundary(isle,n,:)= ij
     endif 

...
class pyOM_cdf(pyOM):
   ...
   def init_snap_cdf(self):
     ...
     if M.my_pe == 0:
       ...
       id=fid.createVariable('rho','f',('Time','zt','yt','xt') )
       id.long_name = 'Density'; id.units='kg/m^3'
       ...
       if M.enable_streamfunction: 
         ...
         id=fid.createVariable('surf_press','f',('Time','yt','xt') )
         id.long_name = 'Surface pressure'; id.units='m/s^2'
         id.missing_value = self._spval

         id=fid.createVariable('press','f',('Time','zt','yt','xt') )
         id.long_name = 'Total pressure'; id.units='m/s^2'
         id.missing_value = self._spval
   ...
   def write_snap_cdf(self):
     ...
     if M.enable_streamfunction: 
       ...
       if M.my_pe == 0: fid.variables['surf_press'][i,:] = a.transpose().astype('f')
     ...
     for k in range(M.nz):
          if not M.enable_streamfunction: 
             b = M.p_hydro[:,:,k] + M.psi[:,:,M.tau-1] 
             a[ M.is_pe-1:M.ie_pe, M.js_pe-1:M.je_pe] = self.mask_me(b,M.maskt[:,:,k])
             self.fortran.pe0_recv_2d(a)
             if M.my_pe == 0: fid.variables['press'][i,k,:,:] = a.transpose().astype('f')

              
          a[ M.is_pe-1:M.ie_pe, M.js_pe-1:M.je_pe] = self.mask_me(M.rho[:,:,k,M.tau-1],M.maskt[:,:,k])
          self.fortran.pe0_recv_2d(a)
          if M.my_pe == 0: fid.variables['rho'][i,k,:] = a.transpose().astype('f')

...
class eady2(pyOM):
   ...
   def set_initial_conditions(self):
     ...
     for k in range(M.nz):
      for j in range(M.yt.shape[0]):
        ...
        M.temp[:,j,k,M.tau-1] =+amp*real(phase*bb[k])*M.maskt[:,j,k]*rho0/(grav*alpha)
   ...
   def make_plot(self):
     ...
     if hasattr(self,'figure'):
       ...
       co=ax.contourf(y,z,a.transpose(),20,cmap='jet')
       ...
       co=ax.contourf(x,y,M.temp[2:-2,2:-2,k,M.tau-1].transpose(),20,cmap='jet')
       ...
       co=ax.contourf(x,y,M.temp[2:-2,2:-2,k2,M.tau-1].transpose(),20,cmap='jet')
       ...
       co=ax.contourf(x,z,a.transpose(),20,cmap='jet')

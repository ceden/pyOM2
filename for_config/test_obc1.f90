...
subroutine set_parameter
 ...
 implicit none
 nx   = int( 64*hRESOLVE ); nz   = int( 18 *vRESOLVE ); ny   = int( 64*hRESOLVE )
 ...
 eq_of_state_type       = 1
 enable_obc_north        = .true.
 enable_obc_south        = .true.
 ...
 enable_diag_snapshots  = .true.; snapint  =  86400./10.
...
subroutine set_initial_conditions
 ...
 implicit none
 integer :: i,j,k
 real*8 :: y,alpha,get_drhodT,t0,L_R
 ...
 do k=1,nz
  do j=js_pe-onx,je_pe+onx
   do i=is_pe-onx,ie_pe+onx
    L_R = (nz*dzt(1)*N_0)/(pi*coriolis_t(i,j) )
    t0= 20.0-N_0**2*zt(k)*rho_0/(grav*alpha)
    temp(i,j,k,:)    = (t0 &
         -1*exp(-xt(i)**2/L_R**2-(yt(j)-L_y/2.)**2/L_R**2)*cos( zt(k)/(nz*dzt(1) )*pi ) &
         +1*exp(-(xt(i)-L_x)**2/L_R**2-(yt(j)-L_y/2.)**2/L_R**2)*cos( zt(k)/(nz*dzt(1) )*pi ) &
          )*maskT(i,j,k)
   enddo
  enddo
 enddo
 ...
 if (enable_obc_south ) then
    do i=is_pe-onx,ie_pe+onx
       psi_wall_south(i) = 10e6*exp( -(xt(i) - 600e3)**2/100e3**2 ) *maskT(i,ny,nz)
    enddo
 endif
 if (enable_obc_north ) then
    do i=is_pe-onx,ie_pe+onx
       psi_wall_north(i) = 10e6*exp( -(xt(i) - 600e3)**2/100e3**2 ) *maskT(i,ny,nz)
    enddo
 endif
